...
from IPv6Django.tools.mock import IPv6TaskMocker


class IPv6VulnerabilityScanner(IPv6TaskBase):
    def __init__(self, origin_file_path_str: str | PathLike[str],
                 work_path_str: str | PathLike[str],
                 params: IPv6TaskParams):
        ...
        self.options_str = params.vuln_params
        self.params = params
    ...
    def scan(self):
        def finished_callback(exit_code):
            try:
                if self.params.mock != 0:
                    nmap_parse_result = IPv6TaskMocker.mock_vuln_scan_data()
                else:
                    # 解析xml为json
        ...
        if self.params.mock != 0:
            nmap_cmd = IPv6TaskMocker.get_mock_command()
            Logger.log_to_file(f"Mock is enabled. Use command {nmap_cmd}", path=self.work_path)
        ...
        self.process_executor.execute(nmap_cmd, finished_callback=finished_callback)
